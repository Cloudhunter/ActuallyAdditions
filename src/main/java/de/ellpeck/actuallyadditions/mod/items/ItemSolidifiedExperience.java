            ...
            if(event.getEntityLiving().worldObj != null && !event.getEntityLiving().worldObj.isRemote && event.getSource().getEntity() instanceof EntityPlayer){
                ...
                if(event.getEntityLiving() instanceof EntityCreature){
                    if(event.getEntityLiving().worldObj.rand.nextInt(10) <= event.getLootingLevel()*2){
                        event.getDrops().add(new EntityItem(event.getEntityLiving().getEntityWorld(), event.getEntityLiving().posX, event.getEntityLiving().posY, event.getEntityLiving().posZ, new ItemStack(InitItems.itemSolidifiedExperience, event.getEntityLiving().getEntityWorld().rand.nextInt(2+event.getLootingLevel())+1)));
                    }
                }
            }
        }
